 ...
 * Provides calendar read/write tools over StreamableHTTP transport.
 * Credentials are injected via environment — never passed to the agent container.
 */
...
if (!CALDAV_URL) {
  console.error('CALDAV_URL is required');
  process.exit(1);
}
      ...
      try {
        ...
        for (const occ of occurrences) {
          ...
          events.push({
            ...
            start: isFloating ? fmtUtc(occ) : occ.toISOString(),
            end: isFloating ? fmtUtc(occEnd) : occEnd.toISOString(),
            isRecurring: true,
          });
        }
        continue;
...
function generateIcal(event: {
  ...
  const lines = [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//NanoClaw//CalDAV MCP//EN',
    'BEGIN:VEVENT',
    `UID:${event.uid}`, `DTSTAMP:${now}`,
...
function createServer(): McpServer {
  ...
  server.tool('list_calendars', 'List all available calendars on the CalDAV server.', {}, async () => {
    try {
      ...
      const list = calendars.map((c) => `- ${c.displayName || '(unnamed)'} [${c.url}]`).join('\n');
  ...
  server.tool(
    'get_events',
    'Get calendar events within a date range. The range is half-open [start, end). To get all events on a single day, set start=that day and end=next day.',
    {
      start: z.string().describe('Start of range (ISO date/datetime)'),
      end: z.string().describe('End of range (ISO date/datetime)'),
      calendar_url: z.string().optional().describe('Specific calendar URL (omit to search all)'),
    },
    async (args) => {
      try {
        const client = await getClient();
        ...
        for (const cal of calendars) {
          ...
          for (const obj of objects) {
            ...
            for (const ev of parseVEvents(obj.data, startDate, endDate)) {
        ...
        const formatted = allEvents.map((e) => {
          let line = `• ${formatLocalDate(e.start)}${e.end ? ` → ${formatLocalDate(e.end)}` : ''}: ${e.summary}${e.isRecurring ? ' (recurring)' : ''}`;
  ...
  server.tool(
    ...
    async (args) => {
      try {
        ...
        const ical = generateIcal({ uid, summary: args.summary, start: args.start, end: args.end, description: args.description, location: args.location });
  ...
  server.tool(
    ...
    async (args) => {
      try {
        const client = await getClient();
        const calendars: DAVCalendarObject[] = await client.fetchCalendarObjects({ calendar: { url: args.calendar_url } as DAVCalendar });
        ...
        await client.updateCalendarObject({ calendarObject: { ...existing, data: generateIcal(updated) } });
  ...
  server.tool(
    ...
    async (args) => {
      try {
        const client = await getClient();
        const objects: DAVCalendarObject[] = await client.fetchCalendarObjects({ calendar: { url: args.calendar_url } as DAVCalendar });
...
app.listen(PORT, () => {
  console.log(`CalDAV MCP server listening on port ${PORT}`);
});
