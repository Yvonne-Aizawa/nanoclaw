/**
 * CalDAV MCP server — runs in-process on the host.
...
function parseVEvents(
  ...
  for (const block of eventBlocks) {
    ...
    const description = parseIcalField(unfolded, 'DESCRIPTION')
      .replace(/\\n/g, '\n')
      .replace(/\\,/g, ',');
...
function formatIcalDate(iso: string): string {
  ...
  if (clean.includes('T'))
    return clean.replace('Z', '') + (iso.endsWith('Z') ? 'Z' : '');
...
export function createCalDavHandler(config: CalDavConfig): InProcessMcpHandler {
  ...
  function createServer(): McpServer {
      ...
      async () => {
        try {
          ...
          const list = calendars
            ...
            .join('\n');
          return {
            content: [{ type: 'text' as const, text: `Calendars:\n${list}` }],
          };
        } catch (err) {
          return {
            content: [
    ...
    server.tool(
      ...
      'Get calendar events within a date range. The range is half-open [start, end). To get all events on a single day, set start=that day and end=next day.',
      {
        ...
        calendar_url: z
          .string()
          .optional()
          .describe('Specific calendar URL (omit to search all)'),
      },
      async (args) => {
        try {
          const client = await getClient();
          ...
          if (allEvents.length === 0) {
            return {
              content: [
                {
                  type: 'text' as const,
                  text: `No events found between ${args.start} and ${args.end}.`,
                },
              ],
            };
          }
    ...
    server.tool(
      ...
      'Create a new calendar event.',
      {
        calendar_url: z
          .string()
          .describe('URL of the calendar to add the event to'),
        ...
        try {
          ...
          return {
            content: [
              {
                type: 'text' as const,
                text: `Event "${args.summary}" created (UID: ${uid}).`,
              },
            ],
          };
        } catch (err) {
    ...
    server.tool(
      ...
      'Update an existing calendar event. Fetch the event first with get_events to get its UID.',
      {
        calendar_url: z
          .string()
          .describe('URL of the calendar containing the event'),
        uid: z.string().describe('UID of the event to update'),
        summary: z.string().optional().describe('New event title'),
        start: z
          .string()
          .optional()
          .describe('New start date/time (ISO format)'),
      ...
      async (args) => {
        try {
          const client = await getClient();
          const calendars: DAVCalendarObject[] =
            await client.fetchCalendarObjects({
              calendar: { url: args.calendar_url } as DAVCalendar,
            });
          const existing = calendars.find(
            (obj) => obj.data && obj.data.includes(`UID:${args.uid}`),
          );
          if (!existing) {
            return {
              content: [
                {
                  type: 'text' as const,
                  text: `Event with UID "${args.uid}" not found.`,
                },
              ],
              isError: true,
            };
          ...
          if (!current) {
            return {
              content: [
                {
                  type: 'text' as const,
                  text: 'Failed to parse existing event.',
                },
              ],
              isError: true,
            };
          ...
          return {
            content: [
              {
                type: 'text' as const,
                text: `Event "${updated.summary}" updated.`,
              },
            ],
          };
        } catch (err) {
